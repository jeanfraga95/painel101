...
MODULE_PATH = '/root/pmaster_module.py'
LOG_PATH = '/root/pmaster_watchdog.log'
...
def get_token():
    """Extrai o token de autenticação do pmaster_module.py."""
    try:
...
def is_module_running():
    """Verifica se o processo pmaster_module.py está rodando."""
    try:
        result = os.popen('pgrep -f "python3.*pmaster_module.py"').read().strip()
        return bool(result)
    except Exception:
        return False
...
def restart_module():
    ...
    os.system('pkill -f "python3.*pmaster_module.py" 2>/dev/null || true')
    ...
    result = os.system(f'nohup python3 {MODULE_PATH} > /root/pmaster_module.log 2>&1 &')
    if result == 0:
...
def ensure_cron():
    ...
    cron_entry = "* * * * * python3 /root/pmaster_watchdog.py"
    ...
    if 'pmaster_watchdog.py' not in result:
        ...
        if cron_entry not in result:
            ...
            lines = [l for l in result.split('\n') if 'pmaster_watchdog.py' not in l]
